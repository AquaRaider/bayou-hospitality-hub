...
export default function HeroSection() {
  ...
  const lock = () => {
    ...
    const keydown = (e: KeyboardEvent) => {
      if (["ArrowDown","ArrowUp","PageDown","PageUp","Space","Home","End"].includes(e.key)) { e.preventDefault(); e.stopPropagation(); }
    };
  ...
  const snapTo = (targetY: number, dir: "down" | "up") => {
    ...
    const finish = () => { if (_ej > 0) window.scrollTo(0, targetY); snapping.current = false; unlock(); lastDir.current = dir; coolUntil.current = performance.now() + COOL; };
    ...
    const step = (now: number) => { const t = Math.min((now - t0) / dur, 1); window.scrollTo(0, animStartY + dist * ease(t)); t < 1 ? requestAnimationFrame(step) : finish(); };
    requestAnimationFrame(step);
  };
  
const finish = () => {
  if (ej > 0) window.scrollTo(0, targetY);
  snapping.current = false;
  unlockInput();

  // 🔔 Tell the rest of the page the hero snap just finished
  window.dispatchEvent(new CustomEvent("heroSnapFinished", { detail: { dir } }));

  lastSnapDir.current = dir;
  refractoryUntil.current = performance.now() + POST_SNAP_COOLDOWN;
};
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      if (inHero()) { e.preventDefault(); if (now < coolUntil.current || Math.abs(e.deltaY) < DELTA) return; snapTo(e.deltaY > 0 ? aboutTop() : 0, e.deltaY > 0 ? "down" : "up"); return; }
      if (inUpZone() && e.deltaY < -DELTA) { e.preventDefault(); if (now < coolUntil.current && lastDir.current === "down") return; snapTo(0, "up"); }
    };
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      if (inHero()) { e.preventDefault(); if (now < coolUntil.current || Math.abs(dy) < 1) return; snapTo(dy > 0 ? aboutTop() : 0, dy > 0 ? "down" : "up"); return; }
      if (inUpZone() && dy < -1) { e.preventDefault(); if (now < coolUntil.current && lastDir.current === "down") return; snapTo(0, "up"); }
    };
  ...
  return (
    <section ref={heroRef} id="home" className="relative isolate min-h-screen flex items-center justify-center overflow-hidden"
      ...
      <div className="relative z-10 mx-auto flex max-w-6xl flex-col items-center px-4 text-center">
        <div className="mb-8 sm:mb-10 will-change-transform"
          ...
          <img src={bayouLogo} alt="Bayou Hospitality" className="w-64 sm:w-72 md:w-80 lg:w-[28rem] h-auto object-contain"
      ...
      <div className="pointer-events-auto absolute bottom-8 left-1/2 z-10 -translate-x-1/2 transition-opacity duration-150"
        ...
        <a href="#about" onClick={(e)=>{e.preventDefault(); if(!snapping.current) snapTo(aboutTop(),"down");}}
      ...
      <div className="absolute inset-x-0 bottom-0 pointer-events-none hero-white-fade will-change-opacity"
